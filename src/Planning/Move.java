...
public class Move {
	...
	//returns difference in ball co-ords
	public int getBallDist(Ball ball, Ball oldball){
		int ballX = ball.getCoors().getX();
		int ballY = ball.getCoors().getY();
		
		int oldballX = oldball.getCoors().getX();
		int oldballY = oldball.getCoors().getY();
		
		int x = (int) Math.abs(oldballX - ballX);
		int y = (int) Math.abs(oldballY - ballY);
...
//		int y = o.getCoors().getY() - robot.getCoors().getY(); // negative y to convert into normal cordinate system
	...
	public int getAngleToBall(Robot robot, Ball b){
		...
		double coord = (ballPos == 0 || ballPos == 3) ? (double) ball.getCoors().getX() : (double) ball.getCoors().getY();
		double a = coord/((double)dist);	
		a = (a > 0) ? a : -a;		

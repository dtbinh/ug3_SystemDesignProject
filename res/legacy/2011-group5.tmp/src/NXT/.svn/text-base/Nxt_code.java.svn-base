...
public class Nxt_code implements Runnable {
	...
	public static void main(String[] args) throws Exception {
		...
		pilot.setMoveSpeed(pilot.getMoveMaxSpeed());
		pilot.setTurnSpeed(pilot.getTurnMaxSpeed());
		...
		while (true) {
			try {
				...
				while (n != QUIT) {
					...
					case FORWARDS:
						if (pilot.getLeftActualSpeed() > 0
								&& pilot.getRightActualSpeed() > 0) {
							break;
						} else {
							pilot.forward();
							break;
					...
					case RIGHT_MOTOR_FORWARDS: // sweep left
						if (pilot.getRightActualSpeed() <= 0) {
							pilot.getRight().forward();
						}
						break;
		...
		while (true) {
			try {
				...
				if (!reacting && (touchA.isPressed() || touchB.isPressed())) {
					...
					reacting = true;
					tempCurSpeed = pilot.getMoveSpeed();
					...
					pilot.setMoveSpeed(tempCurSpeed);

 ...
 * @deprecated
 */
public class ImageProcessorBall {
	...
	public static int[] ballPos(Raster data, Raster bgData, boolean bgSub) {
		...
		for (int y = 0; y < data.getHeight(); y++) {
			...
			for (int x = 0; x < data.getWidth(); x++) {
				...
				if (bgSub) {
					bgData.getPixel(x, y, bgvals);
					if (((Math.abs(red - bgvals[0])) > 15 && Math.abs((green - bgvals[1])) > 15 && Math.abs((blue - bgvals[2])) > 15)) {
						if (red >= 180 && red <= 255 && green >= 0 && green <= 100 && blue >= 0 && blue <= 110) {

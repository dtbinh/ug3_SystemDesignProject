...
public class ViewerTest implements Runnable {
    ...
    public void run() {
		...
		try {
			if(pitch==2) {
				image1 = ImageIO.read(new File("data/pitch2-1.png"));
				image2 = ImageIO.read(new File("data/pitch2-2.png"));
				image3 = ImageIO.read(new File("data/pitch2-3.png")); }
			else {
				image1 = ImageIO.read(new File("data/pitch1-1.png"));
				image2 = ImageIO.read(new File("data/pitch1-2.png"));
				image3 = ImageIO.read(new File("data/pitch1-3.png"));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
